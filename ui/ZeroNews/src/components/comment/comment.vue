<template>
    <div>
        <div class="comment" v-for="comment in page.records" :key="comment.id">
            ...
            <div class="userInfo">
                <span v-text="comment.user.username"> </span><span v-text="comment.createTime.replace('T',' ')"> 2018-10-15 </span><i class="el-icon-edit-outline" @click="subReply(comment)"></i><br>
                <p><span>评论：{{comment.context}}</span></p>
            </div>
            <Reply class="reply" v-for="reply in comment.replyList" :key="reply.id" :reply="reply"/>
            <br>
            <hr>
        </div>
