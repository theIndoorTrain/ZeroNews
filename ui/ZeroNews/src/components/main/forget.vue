<template>
    <div class="bg">
        <el-form  label-width="100px">
            <img src="@/assets/logo.png" class="logo"><h1>找回密码</h1>
    ...
    export default {
        ...
        methods: {
            getCode() {
                ...
                var that = this
                http.get("/forget/"+this.user.email,function(data,status){
                    if(status==true) {
                        if(data==true) {
                            that.isActive = false
                            that.msg("验证码已发送成功！","success")
                        } else {
                            that.msg("账户未注册，请注册账号！","warning")
                            that.$router.push('/sighIn')
                        }
                    }
                })
            },
